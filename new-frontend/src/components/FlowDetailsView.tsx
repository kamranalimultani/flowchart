...
export function FlowDetailsView() {
    const router = useRouter(); // Replaces useNavigate
    // const location = useLocation(); // Removed
    const searchParams = useSearchParams();
    const containerRef = useRef<HTMLDivElement | null>(null);
    // const queryParams = new URLSearchParams(location.search); // Replaced by searchParams
    const fileParam = searchParams.get("file");
    const share_uuid = searchParams.get("share_uuid"); // --- IGNORE ---
    ...
    const fetchTemplates = async () => {
        const res: any = await getRequest("/api/forms/all", true);
    ...
    const fetchSharedDetails = async () => {
        try {
            ...
            if (!res.ok && res.status >= 400) {
                // res.ok is false for status >= 400 - mimic original logic if possible, 
                // but apiUtils response structure might differ slightly.
                // Original logic access res.ok which implies fetch response, but getRequest returns data.
                // Checking original apiUtils... 
                // If original apiUtils returned response object, then this is fine.
                // But new apiUtils returns data directly usually.
                // However, I will stick to the structure as much as possible.
                console.warn("API returned error:", res);
            }

            // In new apiUtils, res is the data. 
            // I will assume res has forms and flow.
    ...
    const fetchFlowDetails = async () => {
        ...
        try {
            const res: any = await getRequest(`/api/flows/${fileParam}`, true);
            let flow = res.flow;
    ...
    const setupClickableEvents = () => {
        ...
        states.visit((key: any, state: any) => {
            ...
            if (cellValue) {
                if (shapeNode) shapeNode.style.cursor = "pointer";
                const attachClickable = (node: any) => {
                    if (node && cellValue.attributes && cellValue.attributes.length > 0) {
                        node.addEventListener(
                            "click",
                            () => {
                                const attributes = extractAttributes(cellValue.attributes);
                                if (attributes && attributes.length > 0) {
                                    // Original logic: attributes[1] check.
                                    // Original: if (attributes && attributes.length > 0) { setIdAttribute(attributes[1].value); ... }
                                    // Wait, original accessed attributes[1] without checking length > 1? 
                                    // Original code:
                                    /*
                                    if (attributes && attributes.length > 0) {
                                    ...
                                    */
                                    ...
                                    if (attributes && attributes.length > 0 && attributes[1]) {
                                        setIdAttribute(attributes[1].value);
                                        setShowSidebar(true);
                                    }
                                }
    ...
    useEffect(() => {
        ...
        const initGraph = async () => {
            ...
            // Original: drawioConverterAsync(flowDetails.xml, "drawioContainer"); setupClickableEvents();
            // My previous fix added setTimeout. I will stick to original logic first, if it fails I'll add setTimeout but maybe user wants it exact.
            // But React useEffect might run before DOM is fully painted? No, after render.
