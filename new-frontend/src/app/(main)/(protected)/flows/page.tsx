...
const FlowCard: React.FC<FlowCardProps> = ({
    ...
    const handleShare = async (id: number) => {
        try {
            const res: any = await postRequest(`/api/flows/share/${id}`, {}, true);
            ...
            console.error("Error sharing flow:", err);
            showError("Failed to share flow. Please try again." + "error:" + err);
        }
    };
    return (
        <>
            ...
            <Card className="relative group w-full aspect-[4/3] shadow-md hover:shadow-lg transition-all rounded-xl flex flex-col overflow-hidden">
                ...
                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4 z-20">
                    <button
                        onClick={(e) => {
                            e.stopPropagation();
                            router.push(`/flow?file=${file_name}`);
                        }}
...
export default function FlowList() {
    ...
    const fetchFlows = async () => {
        try {
            const res: any = await getRequest("/api/flows", true); // Laravel: GET /api/flows
            setFlows(res || []);
        } catch (err: any) {
    ...
    const handleSaveFlow = async () => {
        try {
            ...
            // Refresh flows
            const res: any = await getRequest("/api/flows", true);
            setFlows(res || []);
        } catch (err: any) {
